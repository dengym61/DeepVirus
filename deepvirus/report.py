...
class VirusReport:
    """
    A class for generating reports about VirusTask objects.
    """
    def __init__(
        self, 
        task,
        RT = None):
        """
        Initialize a VirusReport object.

        :param task: a VirusTask object (required).
        :param RT: R=gas constant (in kcal/K/mol) * T=Temperature (in K) (optional).
        :returns: VirusReport object.
        """     
    ...
    def report(
        ...
        if 'models' not in dir(self.task):
            print("Error: Cannot produce report. Invalid VirusTask instance.")
            raise ValueError

        #Check if valid VirusTask with at least one model
        if self.task.models==[]:
            print("Error: Cannot produce report. Invalid VirusTask instance. No fit models.")
            raise ValueError
