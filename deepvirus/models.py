...
from deepvirus.transformation import get_transformation
import itertools
import shutil
import functools
...
class VirusWeightedL1Loss(torch.nn.L1Loss):
    """
...
class VirusGaussianNLLLoss(torch.nn.GaussianNLLLoss):
    """
    Virus version of GaussianNLLLoss with no reduction that accepts fitness weights (1/sigma) rather than var = sigma^2.
    """
    def forward(self, input: Tensor, target: Tensor, weight: Tensor) -> Tensor:
        return F.gaussian_nll_loss(input, target, torch.pow(weight, -2), full = True, eps = 0, reduction = "none")

class VirusModel(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        sos_outputlinear):
        """
        Initialize a VirusModel object.
    ...
    def get_data_loaders(
        ...
        :param data: Dictionary of dictionaries of tensors as output by VirusData.get_data (required).
...
class VirusModelMetadata():
    ...
    def __init__(
        ...
        sos_outputlinear):
        """
        Initialize a VirusModelMetadata object.
        ...
        :param sos_architecture: list of integers corresponding to number of neurons per fully-connected sumOfSigmoids hidden layer (required).
        :param sos_outputlinear: boolean indicating whether final sumOfSigmoids should be linear rather than sigmoidal (required).
        :returns: VirusModelMetadata object.
        """ 
        self.fold = fold
        self.seed = seed
...
class VirusTask():
    ...
    def __init__(
        ...
        """
        Initialize a VirusTask object.

        :param directory: Path to directory where models and results should be saved/loaded (required).
        :param data: An instance of the VirusData class (required unless 'directory' contains a saved task).
    ...
    def save(
        ...
        overwrite = False):
        """
        Save VirusTask object to disk.

        :param overwrite: Whether or not to overwrite previous saved object (default:False).
        :returns: Nothing.
        """ 

        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Task cannot be saved. Invalid VirusTask instance.")
            raise ValueError
    ...
    def new_model(
        ...
        data):
        """
        Create a new VirusModel object.

        :param data: Dictionary of dictionaries of tensors as output by VirusData.get_data (required).
        :returns: A new VirusModel object.
        """ 
        #Create a new model
        model = VirusModel(
    ...
    def get_global_weights(
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Cannot get global weights. Invalid VirusTask instance.")
            raise ValueError
    ...
    def get_linear_weights(
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Cannot get linear weights. Invalid VirusTask instance.")
            raise ValueError
    ...
    def get_additive_trait_weights(
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Cannot get additive trait weights. Invalid VirusTask instance.")
            raise ValueError
    ...
    def grid_search(
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Grid search cannot be performed. Invalid VirusTask instance.")
            raise ValueError

        #Check if valid VirusData
        if not self.data.is_valid_instance():
            print("Error: Grid search cannot be performed. Invalid VirusData instance.")
            raise ValueError
    ...
    def adjust_WT_phenotype(
        ...
        Warning: Use with caution as this method permanently changes the VirusData object.
    ...
    def fit_best(
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Model cannot be fit. Invalid VirusTask instance.")
            raise ValueError

        #Check if valid VirusData
        if not self.data.is_valid_instance():
            print("Error: Model cannot be fit. Invalid VirusData instance.")
            raise ValueError
    ...
    def weights_init_task(
        ...
        input_task):
        """
        Initialize layer weights from supplied VirusTask instance.

        :param fold: Cross-validation fold (required).
        :param model: VirusModel to initialize layer weights (required).
        :param input_task: VirusTask to use for model weight initialization (required).
        :returns: Nothing.
        """ 

        #Check if valid VirusTask
        if 'models' not in dir(input_task):
            print("Error: Cannot initialize weights using supplied VirusTask. Invalid VirusTask instance.")
            raise ValueError
        ...
        #Check if at least one model remaining
        if len(models_subset)==0:
            print("Error: Cannot initialize weights using supplied VirusTask. No models satisfying criteria.")
            raise ValueError
        ...
        print("Weights initialized from VirusTask:")
    ...
    def weights_require_grad(
        ...
        :param model: VirusModel to fix specific layer weights (required).
    ...
    def fit(
        ...
        if type(init_weights) == VirusTask:
        ...
        if loss_function_name == 'WeightedL1':
            loss_function = VirusWeightedL1Loss()
        elif loss_function_name == 'GaussianNLL':
            loss_function = VirusGaussianNLLLoss()
    ...
    def predict_all(
        ...
        :param folds: list of cross-validation folds (default:None i.e. all).
        :param grid_search: Whether or not to include grid_search models (default:False).
        :param data: Optional VirusData object to use for prediction (default:model data).
        ...
        #Check if valid VirusTask
        if 'models' not in dir(self):
            print("Error: Cannot make predictions. Invalid VirusTask instance.")
            raise ValueError

        #Check if valid VirusData
        if not self.data.is_valid_instance():
            print("Error: Cannot make predictions. Invalid VirusData instance.")
            raise ValueError
