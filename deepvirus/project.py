...
class VirusProject():
    ...
    def __init__(
        ...
        sparse_method = None):
        """
        Initialize a VirusProject object.
        ...
        if not self.model_design.empty:
            ...
            if len([i for i in dirlist if i.startswith("task_")])==0:
                ...
                if "saved_models" in dirlist:
                    ...
                    legacy_task = VirusTask(directory = self.directory)
            ...
            for seedi in [int(i) for i in str(self.seed).split(",")]:
                ...
                if not os.path.exists(task_directory):
                    ...
                    self.tasks[seedi] = VirusTask(directory = task_directory)
    ...
    def run_sparse_sig_highestorder_step(
        ...
        for orderi in range(self.max_interaction_order, -2, -1):
            ...
            try:
                ...
                print("Error: Failed to create VirusTask.")
                break
    ...
    def run_cv_task(
        ...
        save_weights = True):
        """
        Run VirusTask and save to disk.

        :param virus_data_args: Dictionary of arguments for VirusData constructor (required).
        :param virus_task_args: Dictionary of arguments for VirusTask constructor (required).
        :param RT: R=gas constant (in kcal/K/mol) * T=Temperature (in K) (optional).
        :param seq_position_offset: Sequence position offset (default:0).
        :param init_weights: Task to use for model weight initialization (optional).
        ...
        #Load virus data
        virus_data = VirusData(**virus_data_args)

        #Create virus project
        ...
        virus_task.grid_search(
            seed = virus_data_args['seed'],
            init_weights = init_weights,
            fix_weights = fix_weights)

        #Fit model using best hyperparameters
        for i in range(virus_data_args['k_folds']):
            virus_task.fit_best(
                fold = i+1, 
                seed = virus_data_args['seed'],
                init_weights = init_weights,
        ...
        #Save all models
        if save_model:
            virus_task.save(overwrite = True)
        ...
        if save_weights:
            ...
            energies_agg = virus_task.get_additive_trait_weights(
                seq_position_offset = seq_position_offset,
                RT = RT,
                aggregate = True,
            ...
            energies_agg_abs = virus_task.get_additive_trait_weights(
                seq_position_offset = seq_position_offset,
                RT = RT,
                aggregate = True,
    ...
    def predict(
        ...
        if not task_id in self.tasks.keys():
            ...
            return
        virus_task = self.tasks[task_id]

        #Model design
        model_design = copy.deepcopy(virus_task.data.model_design)
        ...
        #Set phenotype names
        model_design.phenotype = [virus_task.data.phenotype_names[i-1] for i in list(virus_task.data.model_design.phenotype)]

        #Load virus data
        virus_data = VirusData(
            model_design = model_design,
            max_interaction_order = virus_task.data.max_interaction_order,
            min_observed = 0,
            k_folds = virus_task.data.k_folds,
            seed = virus_task.data.seed,
            validation_factor = virus_task.data.validation_factor, 
            holdout_minobs = virus_task.data.holdout_minobs, 
            holdout_orders = virus_task.data.holdout_orders, 
            holdout_WT = virus_task.data.holdout_WT,
            features = {None: list(virus_task.data.Xohi.columns)},
            ensemble = virus_task.data.ensemble)

        #Reorder feature matrix columns
        virus_data.Xohi = virus_data.Xohi.loc[:,list(virus_task.data.Xohi.columns)]
        virus_data.feature_names = virus_data.Xohi.columns
        #Split into training, validation and test sets
        virus_data.define_cross_validation_groups()
        #Define coefficients to fit (for each phenotype and trait)
        virus_data.define_coefficient_groups(
            k_folds = virus_task.data.k_folds)
        #Ensemble encode features
        if virus_task.data.ensemble:
            virus_data.Xohi = virus_data.ensemble_encode_features()

        #Predictions on all variants for all models
        result_df = virus_task.predict_all(
            data = virus_data,
            save = False)
        #Remove Fold column
        ...
        directory = os.path.join(virus_task.directory, 'predictions')
